...
public class PhotoService extends Generic<Photo> {
	...
	public List<Photo> buscarPorNombre(String nombre) {
		Query q = em
				.createQuery("SELECT a FROM Photo a WHERE a.photoPlantName LIKE :nombre ORDER BY a.photoUploadDate ASC");
	...
	public List<Photo> buscarPorZona(String zona) {
		Query q = em
				.createQuery("SELECT a FROM Photo a WHERE a.photoZone LIKE :zona ORDER BY a.photoUploadDate DESC");
